...
class Student {
      ...
      if (err) {
        ...
        for (let i = 0; i < result.rows.length; i++) {
          ...
          students.push(
            new Student(
              eachResult.id,
              eachResult.first_name,
              eachResult.last_name,
              eachResult.email,
              eachResult.gender,
              eachResult.birth_date
            )
          );
        }
        cb(null, students);
  ...
  static update(student, cb) {
    let query = `UPDATE "Students" SET "first_name" = '${
      student.first_name
    }', "last_name" = '${student.last_name}', "email" = '${
      student.email
    }', "gender" = '${student.gender}', "birth_date" = '${
      student.birth_date
